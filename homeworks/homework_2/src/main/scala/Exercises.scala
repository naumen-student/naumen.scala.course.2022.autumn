object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(el => el % 3 == 0 || el % 5 == 0).sum
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = findSimpleDividers(number).distinct

    def findSimpleDividers(number: Int): Seq[Int] = {
        val firstDiv = (2 to number).find(number % _ == 0).get
        val secondDiv = number / firstDiv
        if (firstDiv != number) firstDiv +: findSimpleDividers(secondDiv) else Seq(firstDiv)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)]): Seq[String] = ballsArray
      .map(ball => {
          val (name, (radius, density)) = ball
          (name, radius * radius * radius * density)
      })
      .toArray
      .sortBy(_._2)
      .map(ball => ball._1)
