object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        for (i <- 2 to number; if number % i == 0) {
            return (Seq(i) ++ primeFactor(number / i)).distinct
        }
        Seq()
    }
    ...
    def isPrime(number: Int): Boolean = {
        ...
        for (i <- 2 to number / 2 + 1) {if(number % i == 0) return false}
        true
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy((ball: (String, (Int, Double))) => {
            val radius = ball._2._1
            val density = ball._2._2
            density * java.lang.Math.PI * java.lang.Math.pow(radius, 3) * 4 / 3
        }).map(ball => ball._1)
    }
