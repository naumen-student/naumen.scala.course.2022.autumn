object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    var res = (2 to scala.math.sqrt(number).toInt).filter(filter)
    if (res.isEmpty && scala.math.abs(number) > 1)
      res = res :+ number
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, fun: Function2[Vector2D, Vector2D, Double], rightVec0: Vector2D, rightVec1: Vector2D) =
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray
      .map(x => (x._1, scala.math.pow(x._2._1, 3) * (4 / 3) * scala.math.Pi * x._2._2))
      .toList
      .sortWith(_._2 < _._2)
      .map(x => x._1)
