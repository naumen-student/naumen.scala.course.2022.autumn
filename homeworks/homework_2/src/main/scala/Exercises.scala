...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        Seq.range(iFrom, iTo + 1).filter(i => i % 3 == 0 || i % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        Seq.range(3, Math.sqrt(n).toInt + 1, 2).foreach(i =>
            while (n % i == 0) {
                res += i
                n /= i
            }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy {
            case (_, (radius, density)) =>
              Math.pow(radius, 3) * Math.PI * 4.0 / 3.0 * density
          }.map(s => s._1)
    }
}