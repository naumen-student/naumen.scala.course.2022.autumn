...
object Exercises {
  ...
  def cata[A](alg: (Seq[A] => A))(e: Fix[Seq]): A =
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
    @tailrec
    def recH(iFrom: Int, iTo: Int, acc: Long): Long =
      if (iFrom > iTo) acc
      else {
        recH(iFrom + 1, iTo, acc + Some(iFrom).filter(_ % 15 == 0).getOrElse(0))
      }
    recH(iFrom, iTo, 0)
  }
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def primeFactorH(number: Int, divs: Set[Int], coPrime: Int): List[Int] = {
      if (number % coPrime == 0)
        primeFactorH(number / coPrime, divs + coPrime, coPrime)
      else
        primeFactorH(number, divs, coPrime + 1)
    }
    primeFactorH(number, Set.empty, coPrime = 2)
  }
  ...
  case class Ball(name: String, density: Double, radius: Int) {
    lazy val weight = radius * radius * Math.PI * density
  }
  ...
  def sortByHeavyweight(
    ...
    ballsArray
      .map {
        case (name, (radius, density)) => Ball(name, density, radius)
      }
      .toList
      .sortBy(_.weight)
      .map(_.name)
