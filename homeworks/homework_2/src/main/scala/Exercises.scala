object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var seqOfMultipliers = Seq[Int]()
        while (copyOfNumber >= divider * divider) {
            while (copyOfNumber % divider == 0) {
                seqOfMultipliers = seqOfMultipliers :+ divider
                copyOfNumber /= divider
            }
            divider += 1
        }
        if (copyOfNumber > 1)
            seqOfMultipliers = seqOfMultipliers :+ copyOfNumber

        seqOfMultipliers.distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map(x => {
            val (name, (radius, density)) = x
            (name, java.lang.Math.PI * math.pow(radius, 3) * density * 4 / 3)
        }).toList.sortBy(x => x._2).map(x => x._1)
    }
}