object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long =
    (iFrom to iTo).filter(digit => digit % 3 == 0 || digit % 5 == 0).foldLeft(0L) {
  ...
  def primeFactor(number: Int): Seq[Int] =
    (2 to number / 2 + 1).foldLeft(List.empty[Int]) {
      case (acc, next) => if (number % next == 0 && !acc.exists(next % _ == 0)) acc :+ next else acc
    }
  ...
  def sumByFunc(
      ...
      rightVec0: Vector2D,
      rightVec1: Vector2D
  ): Double = func(leftVec0, leftVec1) + func(rightVec0, rightVec1)
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.toList
      .sortBy {
        case (_, (radius, density)) => (4 / 3) * Math.PI * Math.pow(radius, 3) * density
      }
      .map {
        case (name, _) => name
      }
