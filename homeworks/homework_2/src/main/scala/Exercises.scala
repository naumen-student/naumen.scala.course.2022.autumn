object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        if (number == 0)
            return Seq()
        var c: Int = 2
        var n: Int = math.abs(number)
        var res: collection.immutable.Set[Int] = Set()
        while (n != 1) {
            while (n % c != 0) {
                c += 1
            }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val ballsWeight = ballsArray
          .map(params => (params._1, (4 / 3) * math.pow(params._2._1, 3) * math.Pi * params._2._2))
        ballsWeight.toSeq.sortWith(_._2 < _._2).map(params => params._1)
    }
}