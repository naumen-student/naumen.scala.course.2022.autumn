object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(i => i % 3 == 0 || i % 5 == 0).sum
  ...
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def primeFactor(number: Int): Seq[Int] = findingDividers(number).distinct

  def findingDividers(number: Int): Seq[Int] = {
    val firstDiv = (2 to number).find(number % _ == 0).get
    val secondDiv = number / firstDiv
    if (firstDiv != number) firstDiv +: findingDividers(secondDiv) else Seq(firstDiv)
  }
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray
      .map(ball => {
        ...
        (name, 4 / 3 * java.lang.Math.PI * radius * radius * radius * density)
      })
      .toArray
      .sortBy(_._2)
      .map(_._1)
