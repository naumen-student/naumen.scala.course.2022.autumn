object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] =
    (2 to number)
      .filter(divisor =>
        number % divisor == 0 &&
          (2 to math.sqrt(divisor).toInt).forall(divisor % _ != 0)
      )
  ...
  def sumScalars(
      ...
      rightVec0: Vector2D,
      rightVec1: Vector2D
  ): Double = sumByFunc(leftVec0, leftVec1, scalar, rightVec0, rightVec1)
  ...
  def sortByHeavyweight(
    ...
    ballsArray.toSeq
      .sortBy { case (_, (radius, density)) =>
        math.pow(radius, 3) * math.Pi * 4d / 3d * density
      }
      .map(_._1)
