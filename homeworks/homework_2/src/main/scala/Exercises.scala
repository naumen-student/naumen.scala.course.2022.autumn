import Exercises.calculateBallWeight
...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var i = 2
        while (i * 2 <= temporaryNum) {
            var flag: Boolean = false
            while (temporaryNum % i == 0) {
                temporaryNum = temporaryNum / i
                flag = true
            }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) =>
      Double, rightVec0: Vector2D, rightVec1: Vector2D) = {
    ...
    */
    ...
    import scala.math._
    def calculateBallWeight(radius: Int, density: Double): Double = java.lang.Math.PI * pow(radius, 3) * density * 4 / 3
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =  {
        val temporarySeq = ballsArray.toSeq
          .map(parameters
        => (parameters._1, calculateBallWeight(parameters._2._1, parameters._2._2)))
          .sortWith(_._2 < _._2)
          .map(parameters => parameters._1)
        println(temporarySeq)
        temporarySeq
    }
