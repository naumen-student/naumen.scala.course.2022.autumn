object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] =
        (for {i <- 2 to number
              }yield i).foldLeft(Set[Int]()){
            case (acc, curr)
                if number % curr == 0 && !acc.exists(curr % _ == 0) => acc + curr
            case (acc, _) => acc
        }.toSeq
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.sortWith((x, y) => x._2._2*Math.PI*3*Math.pow(x._2._1, 3)/4 < y._2._2*Math.PI*3*Math.pow(y._2._1, 3)/4).map(_._1)
}
