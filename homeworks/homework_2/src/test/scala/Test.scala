...
object Test extends TestSuite {
  ...
  val tests: Tests = Tests {
    test("test_divBy3Or7") {
      ...
      assert(
        divBy3Or7(0, 100) ==
          Seq(0, 3, 6, 7, 9, 12, 14, 15, 18, 21, 24, 27, 28, 30, 33, 35, 36, 39,
    ...
    test("test_divBy3Or5") {
      ...
      assert(sumOfDivBy3Or5(16, 17) == 0)
      assert(sumOfDivBy3Or5(1, 5) == Seq(3, 5).sum)
      assert(sumOfDivBy3Or5(9, 16) == Seq(9, 10, 12, 15).sum)
    }

    test("test_primeFactor") {
      assert(List(4, 8, 128, 1024).map(primeFactor).forall(_ == Seq(2)))
    ...
    test("test_sortByHeavyweight") {
      ...
      assert(
        sortByHeavyweight(Map("x" -> (1, 1), "y" -> (2, 2))) == Seq("x", "y")
      )
      assert(
        sortByHeavyweight(
          Map("x" -> (2, 1), "y" -> (1, 2), "z" -> (1, 1))
        ) == Seq("z", "y", "x")
      )
    }
  }
