...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = {
        x match {
            case bool: Boolean =>
                val answer = if (bool) "правда" else "ложь"
                answer
            case _ =>
                x.toString
        }
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        if (x.isInstanceOf[Boolean]) {
            val answer = if (x.asInstanceOf[Boolean]) "правда" else "ложь"
            answer
        } else {
            x.toString
        }
    ...
    def max2(xs: Seq[Int]): Seq[Int] = {
        val answer = if (xs.isEmpty) Seq.empty else Seq(xs.max)
        answer
    }

    def max3(xs: Seq[Int]): Option[Int] = {
        val answer = if (xs.isEmpty) Option.empty else Option(xs.max)
        answer
    }
     ...
     */
    case class MyIterable[T](xs: T*) extends Iterable[T] {
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(MyIterable[Int](x, y))
