...
object Exercises {
    ...
    def sum2(args: Int*): Int = sumIntegers(args)
    def sum3(x: Int, y: Int): Int = sumIntegers(SequenceForTask(x,y))

}
case class SequenceForTask[A](x:A, y:A) extends Iterable[A]{
    val elems: Seq[A] = Seq(x,y);
