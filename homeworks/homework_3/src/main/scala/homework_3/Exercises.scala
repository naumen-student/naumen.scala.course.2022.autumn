...
object Exercises {
  ...
  /**
    ...
    */
  def max1(xs: Seq[Int]): Int = Try(xs.max).getOrElse(0)

  def max2(xs: Seq[Int]): Seq[Int] = Try(xs.max).map(Seq(_)).getOrElse(Seq.empty[Int])
  ...
  def sum3(x: Int, y: Int): Int = sumIntegers(MySuperCollections(x, y))

  case class MySuperCollections[A](el: A*) extends Iterable[A] {
    override def iterator: Iterator[A] = el.iterator
  }
