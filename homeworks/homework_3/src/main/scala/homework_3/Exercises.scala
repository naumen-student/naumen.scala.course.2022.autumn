...
object Exercises {
    ...
    def max3(xs: Seq[Int]): Option[Int] = if(xs.isEmpty) Option.empty else Option(xs.max)
     ...
     */
    class CustomClass(seq: Seq[Int]) extends Iterable[Int] {
        override def iterator: Iterator[Int] = seq.iterator
    }
    def sum1(x: Int, y: Int): Int = sumIntegers(Seq(x,y))
    def sum2(x: Int, y: Int): Int = sumIntegers(List(x,y))
    def sum3(x: Int, y: Int): Int = sumIntegers(new CustomClass(Seq(x,y)))
