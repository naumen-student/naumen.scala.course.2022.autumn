...
object Exercises {
    ...
    def max1(xs: Seq[Int]): Int = {
        if (xs.Empty) Int.MinValue else xs.max
    }
     ...
     */
    class Iterable(elements: Int*) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(elements:_*)

        def sum: Int = elems.sum
    }
