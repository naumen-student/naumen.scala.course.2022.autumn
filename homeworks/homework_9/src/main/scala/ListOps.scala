...
object ListOps {
   ...
   */
  def foldOption[T](f: (T, T) => T)(list: DataList[T]): Option[T] = {
    @tailrec
    def fold(list: DataList[T], acc: T): T = list match {
      case DataList.NonEmptyList(head, tail) => fold(tail, f(acc, head))
  ...
  private def reverse[T](l: DataList[T]): DataList[T] = {
    @tailrec
    def reverseImpl(buffer: DataList[T])(l: DataList[T]): DataList[T] =
      l match {
        case DataList.NonEmptyList(head, tail) => reverseImpl(DataList.NonEmptyList(head, buffer))(tail)
        case DataList.EmptyList => buffer
      }

    reverseImpl(DataList.EmptyList)(l)
  }

  final def filter[T](f: T => Boolean): DataList[T] => DataList[T] = (filterImpl(f)(DataList.EmptyList)(_)).andThen(reverse)
